...
from examples.adspygoogle.adwords.v201302.migration import set_campaign_enhanced
from examples.adspygoogle.adwords.v201302.migration import upgrade_legacy_sitelinks
...
class AdvancedOperations(unittest.TestCase):
  ...
  def setUp(self):
    ...
    if not self.loaded:
      ...
      self.non_ec_campaign_id = util.CreateTestCampaign(client)

  def testUpgradeLegacySitelinks(self):
    upgrade_legacy_sitelinks.main(client, [self.campaign_id])

  def testSetCampaignEnhanced(self):
    set_campaign_enhanced.main(client, self.non_ec_campaign_id)
