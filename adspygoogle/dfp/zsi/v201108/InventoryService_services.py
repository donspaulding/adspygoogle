...
class InventoryServiceLocator:
    InventoryServiceInterface_address = "https://www.google.com:443/apis/ads/publisher/v201108/InventoryService"
...
class InventoryServiceSoapBindingSOAP:
    ...
    # get: getInventoryAdUnitSizes
    def getAdUnitSizes(self, request):
        if isinstance(request, getAdUnitSizesRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getAdUnitSizesResponse.typecode)
        return response
...
getAdUnitSizesRequest = ns0.getAdUnitSizes_Dec().pyclass

getAdUnitSizesResponse = ns0.getAdUnitSizesResponse_Dec().pyclass
