...
class UserServiceLocator:
    UserServiceInterface_address = "https://www.google.com:443/apis/ads/publisher/v201108/UserService"
...
class UserServiceSoapBindingSOAP:
    ...
    # get: getUserCurrentUser
    def getCurrentUser(self, request):
        if isinstance(request, getCurrentUserRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        kw = {}
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="", **kw)
        # no output wsaction
        response = self.binding.Receive(getCurrentUserResponse.typecode)
        return response
...
getCurrentUserRequest = ns0.getCurrentUser_Dec().pyclass

getCurrentUserResponse = ns0.getCurrentUserResponse_Dec().pyclass
