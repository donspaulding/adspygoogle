...
"""This example displays user filter type and user filter criteria type names
and IDs.

Tags: user.getAvailableUserFilterTypes, user.getAvailableUserFilterCriteriaTypes
"""
...
results = user_service.GetAvailableUserFilterTypes()

# Display user filter types.
if results:
  for filter_type in results:
    print ('User filter type with name \'%s\' and ID \'%s\' was found.'
           % (filter_type['name'], filter_type['id']))
else:
  print 'No user filter types found.'
